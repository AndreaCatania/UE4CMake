...
//    private string[] m_libraries;
	...
	public bool addRules(ModuleRules rules, bool useSystemCompiler)
	{
		...
		if (values.ContainsKey("includes"))
		{
			...
			foreach (string include in includes)
			{
				...
				Console.WriteLine("[UE4CMake] Add include path: " + include);
		...
		if (values.ContainsKey("binaryDirectories"))
		{
			...
			foreach (string binaryDirectory in binaryDirectories)
			{
				...
				Console.WriteLine("[UE4CMake] Add library path: " + binaryDirectory);
		...
		if (values.ContainsKey("libraries"))
		{
			...
			foreach (string library in libraries)
			{
				...
				Console.WriteLine("[UE4CMake] Add library: " + library);
	...
	private string CreateCMakeConfigCommand(ReadOnlyTargetRules target, ModuleRules rules, string buildDirectory,
		string buildType, bool useSystemCompiler)
	{
	...
	private void generateUnixToolchain(ReadOnlyTargetRules target, GeneratorInfo generatorInfo, string path,  bool useSystemCompiler)
	{
		...
		contents = contents.Replace("@ARCH_NAME@", "x86_64-unknown-linux-gnu");

		string cxx_flags = "";
		cxx_flags += " -mno-avx";
		cxx_flags += " -mno-avx2";
		
		contents = contents.Replace("@C_FLAGS@", cxx_flags + "");
		contents = contents.Replace("@CXX_FLAGS@", cxx_flags + " -fvisibility=default -I/home/andrea/Workspace/Linux_Unreal_Engine_5.4.4/Engine/Source/ThirdParty/Unix/LibCxx/include/c++/v1");
		contents = contents.Replace("@LINKER_FLAGS@", "-nodefaultlibs -L/home/andrea/Workspace/Linux_Unreal_Engine_5.4.4/Engine/Source/ThirdParty/Unix/LibCxx/lib/Unix/x86_64-unknown-linux-gnu/");
...
public class CMakeTarget : ModuleRules
{
	...
	public static bool add(ReadOnlyTargetRules target, ModuleRules rules, string targetName, string targetLocation,
		string args, bool useSystemCompiler = false)
	{
